...
public class FlowUtil
{
    ...
    public static void updateFlowDeploymentPlan(final List<MetaInfo.Flow.Detail> deploymentPlan, final UUID flowId) throws MetaDataRepositoryException {
        final MetaInfo.Flow flow = (MetaInfo.Flow)FlowUtil.metadataRepository.getMetaObjectByUUID(flowId, WASecurityManager.TOKEN);
        flow.setDeploymentPlan(deploymentPlan);
        FlowUtil.metadataRepository.updateMetaObject(flow, WASecurityManager.TOKEN);
    }
    
    public static MetaInfo.StatusInfo getCurrentStatus(final UUID appId) throws MetaDataRepositoryException {
        return FlowUtil.metadataRepository.getStatusInfo(appId, WASecurityManager.TOKEN);
    }
    
    public static MetaInfo.StatusInfo.Status getCurrentStatusFromAppManager(final UUID appId) throws Exception {
        final AppManagerRequestClient appManagerRequestClient = new AppManagerRequestClient(WASecurityManager.TOKEN);
    ...
    public static MetaInfo.StatusInfo.Status updateFlowActualStatus(final UUID appId, final MetaInfo.StatusInfo.Status newStatus) throws MetaDataRepositoryException {
        MetaInfo.StatusInfo status = FlowUtil.metadataRepository.getStatusInfo(appId, WASecurityManager.TOKEN);
        if (status == null) {
            status = new MetaInfo.StatusInfo();
            final MetaInfo.Flow flow = (MetaInfo.Flow)FlowUtil.metadataRepository.getMetaObjectByUUID(appId, WASecurityManager.TOKEN);
        ...
        FlowUtil.metadataRepository.putStatusInfo(status, WASecurityManager.TOKEN);
        return newStatus;
    }
    
    public static MetaInfo.Flow getFlow(final UUID flowId) throws MetaDataRepositoryException {
        final MetaInfo.MetaObject object = FlowUtil.metadataRepository.getMetaObjectByUUID(flowId, WASecurityManager.TOKEN);
