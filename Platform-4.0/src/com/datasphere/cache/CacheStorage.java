...
public class CacheStorage<K, V>
{
    ...
    public Map<String, HIndex<?, K, V>> indices;
    private long cacheHit;
    private long cacheMiss;
    
    public CacheStorage(final String cacheName, final int numPartitions) {
        this.indices = new HashMap<String, HIndex<?, K, V>>();
        this.cacheHit = 0L;
        this.cacheMiss = 0L;
    ...
    // 添加索引
    public <I> void addIndex(final String name, final HIndex.Type type, final HIndex.FieldAccessor<I, V> accessor, final IPartitionManager pm) {
        final HIndex<I, K, V> index = new HIndex<I, K, V>(type, accessor, pm);
        this.indices.put(name, index);
    }
    
    public <I> Map<K, V> getIndexedEqual(final String name, final I value) {
        final HIndex<I, K, V> index = (HIndex<I, K, V>)this.indices.get(name);
        if (index == null) {
    ...
    public void put(final int partId, final K key, final V value) {
        if (key == null || value == null) {
            ...
            if (part != null) {
                part.put(key, value);
                for (final Map.Entry<String, HIndex<?, K, V>> indexEntry : this.indices.entrySet()) {
                    final HIndex<?, K, V> index = indexEntry.getValue();
                    try {
    ...
    public boolean remove(final int partId, final K key) {
        ...
        if (key == null) {
            ...
            if (value == null) {
                ...
                removed = true;
                for (final HIndex<?, K, V> index : this.indices.values()) {
